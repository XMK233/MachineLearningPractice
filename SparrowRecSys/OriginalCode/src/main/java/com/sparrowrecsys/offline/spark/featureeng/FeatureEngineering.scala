...
//  }
  //这个地方, 原本的代码是用的花括号. 这里我们给它换成圆括号, 也能正常运行.
  // 这里的udf应该是spark.ml.feature里面的函数, 大概的作用就是返回一个函数, 或者说把一个类似于匿名函数的东西
  // 给它转为一个用户定义函数.
  // 所以这也就是我为什么讨厌使用import xxx._ 这种形式的引用. 酱紫一搞, 你都不知道你这个函数是来自哪里的.
  // 非常不便于查资料. 哎.
  val double2vec: UserDefinedFunction = udf (
      (value: Double) => org.apache.spark.ml.linalg.Vectors.dense(value)
    )
